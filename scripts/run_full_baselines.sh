...
FLEXROUND_LOG_DIVISOR_BOUND=${FLEXROUND_LOG_DIVISOR_BOUND:-inf}
FLEXROUND_GROUP_SIZE=${FLEXROUND_GROUP_SIZE:-$GROUP_SIZE}
  ...
  for SCHEME in $SCHEMES; do
    for ASSIGNMENT in $ASSIGNMENTS; do
      ...
      if [[ "$ASSIGNMENT" == "qronus" ]]; then
        METHOD_GROUP_SIZE=-1
      elif [[ "$ASSIGNMENT" == "flexround" ]]; then
        METHOD_GROUP_SIZE="$FLEXROUND_GROUP_SIZE"
      fi
